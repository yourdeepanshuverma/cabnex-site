...
const VendorEditCar = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    airConditioning: 'AC',
    features: [
      { key: 'ac', label: 'Air Conditioning', enabled: false },
    ...
    const fetchCar = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          const featuresMap = new Map(formData.features.map(f => [f.key, f]));
          carData.features.forEach(featureKey => {
            if (featuresMap.has(featureKey)) {
              featuresMap.get(featureKey).enabled = true;
            }
          });
          ...
          setFormData({
            ...formData,
            ...carData,
            airConditioning: carData.airConditioning ? 'AC' : 'Non-AC',
            status: carData.status === 'active' ? 'available' : carData.status,
            insuranceExpiry: carData.insuranceExpiry ? new Date(carData.insuranceExpiry).toISOString().split('T')[0] : '',
            pollutionExpiry: carData.pollutionExpiry ? new Date(carData.pollutionExpiry).toISOString().split('T')[0] : '',
            features: Array.from(featuresMap.values()),
            images: [], // Reset images, will handle separately
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Car updated successfully! Redirecting to car list...');
      ...
      const errorMsg = err.response?.data?.message || 'Failed to update car. Please try again.';
  ...
  const validateStep = (step) => {
    ...
    return formData.insuranceExpiry && formData.pollutionExpiry;
  };
  ...
  return (
    <div className="space-y-8 dark:bg-gray-900">
      ...
      <div className="form-container p-6">
        <h3 className="text-2xl font-grotesk font-bold text-gray-800 dark:text-gray-200 mb-6">Edit Car</h3>
                ...
                <div>
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-4">
                    {existingImages.map((img, idx) => (
                      <img key={idx} src={img.url} alt={`Existing ${idx}`} className="w-full h-32 object-cover rounded-md shadow-md" />
                    ))}
                  </div>
                </div>
                <div>
            ...
            {currentStep < 3 ? (
              ...
              >
                {loading ? (
                  ...
                  <>
                    <FaSave className="mr-2 w-4 h-4" /> Update Car
                  </> 
                )}
              </button>
            )}
