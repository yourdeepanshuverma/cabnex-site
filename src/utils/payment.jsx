...
export const loadRazorpay = async ({
  ...
  const initiatePayment = async () => {
    try {
      ...
      const { data: keyData } = await axios.get(`${API_URL}/transaction/get-razorpay-key`, {
          ...
          try {
            ...
            const verifyPayload = {
              ...
              destinations: Array.isArray(destinations)
                ? destinations
                : destinations
                ? [typeof destinations === 'object' ? destinations : { address: destinations, place_id: null }]
                : [],
              returnDateTime,
              distance: distance || 0,
              totalAmount,
            ...
            const verifyRes = await axios.post(`${API_URL}/transaction/verify-payment`, verifyPayload, {
              withCredentials: true,
            });
            ...
            if (verifyRes.data.success) {
              ...
              const bookingResponse = encodeURIComponent(JSON.stringify(verifyRes.data));
setTimeout(() => {
  window.location.href = `${BASE_URL}/success?data=${bookingResponse}`;
}, 2000);
            } else {
              toast.error(verifyRes.data.message || 'Verification failed! Redirecting to failure page...');
              setTimeout(() => {
